import { ref } from 'vue'

interface Partner {
  id: number
  title: string
  image: string
  link?: string
}

export const usePartners = () => {
  const partners = ref<Partner[]>([])
  const loading = ref(true)
  const error = ref<string | null>(null)

  const fetchPartners = async () => {
    try {
      loading.value = true
      error.value = null

      // Fetch partners from WordPress API
      const partnersResponse = await fetch(
        'https://logic-design-solutions.com/wp-json/wp/v2/partners?per_page=100'
      )

      if (!partnersResponse.ok) {
        throw new Error(`Failed to fetch partners: ${partnersResponse.status}`)
      }

      const partnersData = await partnersResponse.json()

      // Get featured media IDs
      const mediaIds = partnersData
        .filter((partner: any) => partner.featured_media)
        .map((partner: any) => partner.featured_media)

      // Fetch media data for all featured images
      let mediaMap: Record<number, any> = {}

      if (mediaIds.length > 0) {
        const uniqueMediaIds = Array.from(new Set(mediaIds)) // Remove duplicates
        const mediaResponse = await fetch(
          `https://logic-design-solutions.com/wp-json/wp/v2/media?include=${uniqueMediaIds.join(',')}`
        )

        if (mediaResponse.ok) {
          const mediaData = await mediaResponse.json()
          // Create a map of media ID to media object
          mediaMap = mediaData.reduce((acc: any, media: any) => {
            acc[media.id] = media
            return acc
          }, {})
        }
      }

      // Transform partners data to match our interface
      partners.value = partnersData.map((partner: any) => {
        const media = mediaMap[partner.featured_media]

        return {
          id: partner.id,
          title: partner.title?.rendered || 'Partner',
          image: media?.source_url || '/assets/images/brand/AMD_Embedded_Select.png',
          link: partner.link || '#'
        }
      })
    } catch (err) {
      error.value = err instanceof Error ? err.message : 'Unknown error'
      console.error('Error fetching partners:', error.value)
    } finally {
      loading.value = false
    }
  }

  return {
    partners,
    loading,
    error,
    fetchPartners
  }
}
